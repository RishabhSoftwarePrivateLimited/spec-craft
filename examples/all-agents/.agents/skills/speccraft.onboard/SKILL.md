---
name: speccraft.onboard
description: Onboard an existing (brownfield) application by discovering its architecture from code. Use when user runs $speccraft.onboard or asks to onboard/import an existing codebase into this workflow.
---
...
Input: read the optional path to existing code the user supplied after invoking this skill (only needed if it isn't already inside `src-code-frontend/`/`src-code-backend/`).
