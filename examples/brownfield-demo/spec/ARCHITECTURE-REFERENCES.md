...
Stack discovered via `/speccraft.onboard` against pre-existing code — `Layer Scope: both` (React 19+Vite frontend, Express 5 backend, no auth). See `spec/architecture/ARCH-DECISIONS.md` for the full decision log and its Evidence columns.

| Document | Scope | Layer | Apply In |
|---|---|---|---|
| `spec/architecture/ARCH-DECISIONS.md` | Discovered stack decisions — `AD-FRONTEND-001`..`008` (framework, UI, state, forms, auth, styling, typing, testing); `AD-BACKEND-001`..`007` (framework, data access, auth, API style, messaging, typing, testing); `AD-X-001`..`003` (API contract format, CORS, token format) | frontend / backend / cross-cutting | Every LLD (`LLD-FRONTEND` and/or `LLD-BACKEND`) and implementation task — read before designing or coding, filtered to the rows matching the task's or LLD's `Layer` |
| `spec/architecture/FRONTEND-STRUCTURE.md` | Discovered routing conventions (none — single screen, no router) and module boundaries (`api/` → `components/` → `types/`, one-way dependency flow) | frontend | Frontend LLD creation, `/speccraft.decompose`, `/speccraft.implement` for `Layer: frontend` tasks |
| `spec/architecture/BACKEND-STRUCTURE.md` | Discovered layering (`routes` → `controllers` → `services` → `repositories`) and data/integration strategy (in-memory only, no external systems found) | backend | Backend LLD creation, `/speccraft.decompose`, `/speccraft.implement`/`/speccraft.integration-test` for `Layer: backend` tasks |
