...
changed_files:
  ...
  - spec/commands/speccraft.scaffold.md
  - CLAUDE.md
  - spec/SPEC-VERSION.md

---

## Note — v0.1.1 (2026-08-12)

Locks this project's stack for both layers ("Before You Start" steps 2–5, greenfield-demo): `## Layer Scope` = `both`, `AD-FRONTEND-001..007` (Vite+React+TS, no UI/state library, Vitest), `AD-BACKEND-001..006` (Express+TS, in-memory repository, `node --test`+`supertest`), `AD-X-001..003` (OpenAPI contract convention, CORS, no-auth token format) in `spec/architecture/ARCH-DECISIONS.md`; adds two supporting architecture spec files, `FRONTEND-STRUCTURE.md` (routing + module boundaries) and `BACKEND-STRUCTURE.md` (layering + data/integration strategy), both indexed in `spec/ARCHITECTURE-REFERENCES.md`; fills in `spec/commands/speccraft.scaffold.md`'s per-layer placeholder sections to match. `spec/business/tasks/BRD.md` and `spec/business/tasks/TASK-TRACKER-001.md` were also added this session but are not on the tracked-files list (business specs aren't cache-tracked). See `examples/LIVE-EXAMPLES-PLAN.md` for the full context — this is the greenfield demo project for the `speccraft` package itself, not a change to the `speccraft` template source.
